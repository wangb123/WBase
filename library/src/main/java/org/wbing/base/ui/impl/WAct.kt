...
import org.wbing.base.ui.WView

/**
 * Activity的基础类
 */
abstract class WAct<Binding : ViewDataBinding> : AppCompatActivity(), WView<Binding>, Call<Message, Boolean> {
    companion object {
    ...
    /**
     * 生命周期函数，activity创建，获取参数，填充布局 功能
     */
    ...
    override fun onBackPressed() {
        if (isTaskRoot) {
            ...
            if (millis - lastBackPressTime > BACK_DOUBLE_CLICK_INTERVAL) {
                Toast.makeText(getContext(), getText(R.string.w_base_double_back_ensure_hint), Toast.LENGTH_SHORT).show()
